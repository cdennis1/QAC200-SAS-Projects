...
DATE: Thursday, January 08, 2015     TIME: 11:23:22 AM
PROJECT: cdennis_SAS_project_010814
PROJECT PATH: P:\QAC\qac200\students\cdennis\cdennis_SAS_project_010814.egp
---------------------------------------- */

/* Unable to determine code to assign library DATA1 on Local */
...
%LET _CLIENTPROJECTPATH='P:\QAC\qac200\students\cdennis\cdennis_SAS_project_010814.egp';
%LET _CLIENTPROJECTNAME='cdennis_SAS_project_010814.egp';
...
/* -------------------------------------------------------------------
   ...
   Generated on: Thursday, January 08, 2015 at 11:22:39 AM
...
PROC DATASETS NOLIST NODETAILS; 
   CONTENTS DATA=DATA1.CDENNIS OUT=WORK.SUCOUT1;

RUN;

DATA WORK.CONTContentsForCDENNIS(LABEL="Contents Details for CDENNIS");
   SET WORK.SUCOUT1;
RUN;
...
CREATE VIEW WORK.SCVIEW AS 
	SELECT DISTINCT memname LABEL="Table Name", 
			...
			charset LABEL="Char. Set", 
			protect LABEL="Password Protected", 
			typemem LABEL="Data Set Type" FROM WORK.CONTContentsForCDENNIS
	ORDER BY memname ; 

CREATE TABLE WORK.SCTABLE AS
	SELECT * FROM WORK.SCVIEW
		WHERE memname='CDENNIS';
QUIT;
...
PROC SORT DATA=WORK.CONTContentsForCDENNIS OUT=WORK.CONTContentsForCDENNIS;
   BY memname name;
RUN;
...
DROP TABLE WORK.SCTABLE;
CREATE TABLE WORK.SCTABLE AS
	SELECT * FROM WORK.CONTContentsForCDENNIS
		WHERE memname='CDENNIS';
QUIT;
